...
class PrivateClass{
	 ...
	 */
class DefaultClass{
	// 같은 패키지 내의 다른 클래스들에게 접근 가능
	int myField;
	void myMethod() {
		// 같은 패키지 내 다른 클래스에서 생성 시
		//The type DefaultClass is already defined
	}
}

/*
 3) protected
 : 동일한 패키지 내의 모든 클래스와 다른 패키지의 하위 클래스에서 접근가능
 */
class ProtectedClass extends ParentClass{
	protected int myField;
	
	public ProtectedClass() {
		// protected 속성과 메소드일 경우
		// >> 상속받은 클래스에서 다른 패키지더라도 접근 가능
		System.out.println(protectedField);
	}
}

// 어떤 클래스이던지 어떤 패키지 내에 있던지 접근 가능
public class A_Modifier {
	static String name= "gwon";
